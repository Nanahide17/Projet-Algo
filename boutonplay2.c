...
int main(int argc, char* argv[]) {
    ...
    SDL_Window* window = SDL_CreateWindow("Images", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 600, 400, 0);

    SDL_Renderer* renderer = SDL_CreateRenderer(window, -1, 0);

    SDL_Surface* image1 = IMG_Load("play1.png");
    SDL_Texture* texture1 = SDL_CreateTextureFromSurface(renderer, image1);
    SDL_FreeSurface(image1);

    SDL_Surface* image2 = IMG_Load("play2.png");
    ...
    SDL_Surface* image4 = SDL_LoadBMP("vert.bmp");
    SDL_Texture* texture4 = SDL_CreateTextureFromSurface(renderer, image4);
    SDL_FreeSurface(image4);

    SDL_Rect imageRect1 = {50, 50, 200, 200};
    ...
    while (!done) {
        ...
        SDL_RenderCopy(renderer, texture4, NULL, NULL); // Afficher l'image verte en arrière-plan
